...
const HistoryPage: React.FC = () => {
  ...
  const { data, isFetching } = useGetAccountHistoryQuery({
    page,
    limit,
    search: searchQuery,
    dateRange,
        ...
        data.data.forEach((newEntry) => {
          if (updatedEntries.has(newEntry.date)) {
            ...
            updatedEntries.set(newEntry.date, {
              ...existingEntry,
              entries: [...existingEntry.entries, ...newEntry.entries],
            });
          } else {
  ...
  return (
    <Box
      sx={{ maxWidth: { md: '60%', xs: '85%' }, flexGrow: 1 }}
      mx={'auto'}
      py={4}
    >
      <Typography
      ...
      <Search
        ...
        onDateRangeChange={(dateRange: DateRangeOptions) =>
      ...
      >
        {!isFetching && allEntries.length === 0 ? (
          <Typography
            sx={{ textAlign: 'center', color: 'grey.600', fontSize: 18 }}
          >
            No data found
          </Typography>
        ) : (
          allEntries.map(
            (historyDateBasedEntries: HistoryEntriesByDate, index: number) => (
              <Box
                key={index}
                sx={{
                  mb: '30px',
